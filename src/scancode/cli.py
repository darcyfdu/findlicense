...
def save_logs(text,output_file):
    ...
    fo = open("logs/"+filename+'.log', "a+")
    fo.write(scantime+' '+text+'\n')
    fo.close()
...
def scancode(ctx,
    ...
    try:
        ...
        if not quiet:
            ...
            save_logs('Saving results.',output_file)
        ...
        if not os.path.isfile(format):
            ...
            for template_chunk in as_template(results, files_count,output_file,template=format):
                try:
                    ...
                    save_logs(extra_context,output_file)
                    e.args += (extra_context,)
                    raise e
        return

    write_formatted_output(scanners, files_count, version, notice, results, format, options, input, output_file, echo_stderr,save_logs)
